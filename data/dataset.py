...
class ValueDataset(Dataset):
    def __init__(self, examples, fields, dictionary, is_train=False):
...
class DictionaryDataset(Dataset):
    def __init__(self, examples, fields, key_fields, is_train=False):
        ...
        self.key_dataset = Dataset(key_examples, key_fields, is_train=self.is_train)
        self.value_dataset = ValueDataset(value_examples, value_fields, dictionary, is_train=self.is_train)
...
class PairedDataset(Dataset):
    ...
    def image_dictionary(self, fields=None):
        if not fields:
            fields = self.fields
        dataset = DictionaryDataset(self.examples, fields, key_fields='image', is_train=self.is_train)
        return dataset
...
class Flickr30k(PairedDataset):
    def __init__(self, image_field, text_field, img_root, ann_root, id_root=None, use_restval=True,
                 cut_validation=False):
        roots = {}
        roots['train'] = {
            'img': os.path.join(img_root, 'Trainflickr30k'),
            'cap': os.path.join(ann_root, 'dataset_flickr30k.json')
        }
        roots['val'] = {
            'img': os.path.join(img_root, 'Valflickr30k'),
            'cap': os.path.join(ann_root, 'dataset_flickr30k.json')
        }
        roots['test'] = {
            'img': os.path.join(img_root, 'Testflickr30k'),
            'cap': os.path.join(ann_root, 'dataset_flickr30k.json')
        }
        # roots['trainrestval'] = {
        #     'img': (roots['train']['img'], roots['val']['img']),
        ...
        with nostdout():
            self.train_examples, self.val_examples, self.test_examples = self.get_samples(roots, ids)
        examples = self.train_examples + self.val_examples + self.test_examples
        super(Flickr30k, self).__init__(examples, {'image': image_field, 'text': text_field})
    ...
    def get_samples(cls, roots, ids_dataset=None):
        ...
        val_samples = []
        test_samples = []
        img_root = "/home/gujiayang/data/dataset/imagecaption/Flickr30k"
        info_all = json.load(open('/home/gujiayang/data/dataset/imagecaption/Flickr30k/dataset_flickr30k.json'))['images']

        for _idx in range(len(info_all)):
            split = info_all[_idx]['split']
            info_single = info_all[_idx]

            all_captions = info_single['sentences']
            for sid in range(5):
                caption =  all_captions[sid]['raw']
                
                if split == 'train':
                    filename = "flickr30K/images/Trainflickr30k/"+info_single['filename']
                    example = Example.fromdict({'image': filename, 'text': caption})
                    train_samples.append(example)
                elif split == 'val':
                    filename = "flickr30K/images/Valflickr30k/"+info_single['filename']
                    example = Example.fromdict({'image': filename, 'text': caption})
                    val_samples.append(example)
                elif split == 'test':
                    filename = "flickr30K/images/Testflickr30k/"+info_single['filename']
                    example = Example.fromdict({'image': filename, 'text': caption})
                    test_samples.append(example)
