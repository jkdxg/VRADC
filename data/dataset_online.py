...
class COCO_online(PairedDataset):
    def __init__(self, image_field, text_field, img_root, ann_root, id_root=None, use_restval=True,
        ...
        roots['test'] = {
            ...
            'cap': os.path.join(ann_root, 'captions_test2015_online.json')  # captions_val2014.json
        }
        roots['trainrestval'] = {
        ...
        if id_root is not None:
            ...
            dev = np.load(os.path.join(id_root, 'coco_dev_ids.npy'))
            restval = np.load(os.path.join(id_root, 'coco_restval_ids.npy'))
            test_as_val = np.load(os.path.join(id_root, 'coco_test_ids.npy'))
            ids['val'] = np.concatenate([dev,restval,test_as_val],axis=0)
            
            if cut_validation:
                ids['val'] = ids['val'][:5000]
            ids['test'] = np.load(os.path.join(id_root, 'coco_test_online.npy')) # coco_test_ids.npy
            ids['trainrestval'] = (
                ids['train'],
        ...
        super(COCO_online, self).__init__(examples, {'image': image_field, 'text': text_field})
