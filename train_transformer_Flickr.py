import random
from data import ImageDetectionsField, TextField, RawField, ImageField
from data import COCO, DataLoader
from torch.utils.data import DistributedSampler
from data.dataset import Flickr30k
import evaluation
...
from models.transformer.encoder_entry import build_encoder,build_random_init_encoder
...
def _changeConfig(config, worldSize):
    ...
    config.xe_base_lr = config.xe_base_lr * worldSize
    config.rl_base_lr = config.rl_base_lr * worldSize


def _generalConfig(rank: int, worldSize: int):
    os.environ["MASTER_ADDR"] = "localhost"
    os.environ["MASTER_PORT"] = "43568" #任意一个没被占用的端口号
...
def train(rank, worldSize, args):
    ...
    # Create the dataset
    dataset = Flickr30k(image_field, text_field, 'coco/images/', args.annotation_folder, args.annotation_folder)
    ...
    # backbone = build_random_init_encoder(args)
    vlad_encoder = NetVLAD()
    att_encoder = encoders.AfterVladEncoder(N = 5,padding_idx = 0)
    # decoder = TransformerDecoderLayer(len(text_field.vocab), 54, 3, text_field.vocab.stoi['<pad>'], d_model=1024)
    decoder = TransformerDecoderLayer(len(text_field.vocab), 80, 3, text_field.vocab.stoi['<pad>'], d_model=512)

    torch.cuda.set_device(rank)
    model = Transformer(text_field.vocab.stoi['<bos>'], backbone, vlad_encoder, att_encoder, decoder)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--exp_name', type=str, default='swintransformer_v3_bestpara_random_init')
    parser.add_argument('--exp_name', type=str, default='swintransformer_v3_bestpara_Flickr30K')
    ...
    parser.add_argument('--refine_epoch_rl', type=int, default=14)
    ...
    parser.add_argument('--img_root_path', type=str, default='/home/gujiayang/data/dataset/imagecaption/Flickr30k')
