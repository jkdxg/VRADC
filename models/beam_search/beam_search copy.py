    ...
    def apply(self, visual: utils.TensorOrSequence, out_size=1, return_probs=False, **kwargs):
        ...
        with self.model.statefulness(self.b_s):
            for t in range(self.max_len):
                ...
                att = torch.mean(att, dim=1).view(att.shape[0], -1)
                if t == 0:
                    att = att.repeat(5, 1)
                atts.append(att.unsqueeze(1))
        atts = torch.cat(atts, dim=1)  # zjk
        atts = atts.view(-1, self.beam_size, atts.shape[1], atts.shape[-1])[:, :out_size].squeeze(1)
        ...
        if return_probs:
            ...
            return outputs, log_probs, atts #zjk
